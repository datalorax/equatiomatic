...
The purpose of  [{equatiomatic}](https://datalorax.github.io/equatiomatic/) is to help you from model output, like the `lm()` output above,
...
(importantly) RMarkdown documents rendered to either HTML or PDF formats.
...
## Ordinal Regression
For these examples, we'll use wine tasting data from the [{ordinal}](https://github.com/runehaubo/ordinal) package. If you don't laready have this package, you can download it with
...
You can fit ordinal response models with either `MASS::polr` or `ordinal::clm`, and {equatiomoatic} works with either, and with logistic or probit link functions.
