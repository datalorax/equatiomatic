...
A more complete overview of **{latex2exp}** is available through [the package vignette](https://cran.r-project.org/web/packages/latex2exp/vignettes/using-latex2exp.html). A few important notes:

As mentioned above, `\operatorname` is unfortunately not supported by `latex2exp::Tex()` so if you are getting an error or missing operators, check that `ital_vars = FALSE` is set in `equatiomatic::extract:eq()`. Additionally, as shown above, underscores and other parts of the equation may not directly translate. If you run into issues, please do file an issue. It is likely that future development will support helper functions for plotting.

**Multi-line equations require special handling and have a reference section below.**
...
Plotmath does not support new lines in expressions. The user has two (2) options, both of which are imperfect:
...
- `&` control alignment and will likely be used in conjunction with the `\\begin` and `\\end` statements above. `{alignat}` uses multiple `&` to represent different alignment points (e.g. `&&` for the second alignment point). These should also be removed.
...
Following the parsing notes in the previous section, the equation can be split using `strsplit()` on the new lines (`\\`) for multi-line equations. The user can then manually place each line using `graphics::text()` or `graphics::mtext()` in base R or `annotate()` in **{ggplot2}**.

## Future Development

At this point, we have no plans to build a native parser in **{equatiomatic}** because **{latex2exp}** should work for most equations. If there are common problems (such as the underscore issue shown above), we may build out some helper functions or additional argument(s) for `extract_eq()` to help with plotting. Please let us know if there is something you can't do with **{latex2exp}** by submitting an issue.

Thanks again to the [#rstats](https://twitter.com/hashtag/rstats) community. The **{latex2exp}** package is a great compliment to **{equatiomatic}**. We also appreciate the feedback from **{equatiomatic}** users. Please let us know if you run into any issues (preferably with a reproducible example) in the issues section of the package GitHub page. 
